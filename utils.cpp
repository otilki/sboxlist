...
void graphe_to_file(map<int, RandomAccessSet<function_t> > *graphe,
                    string graphe_name)
{
  string name;
  int tot_number = 0;
  ...
  */
  ...
  file_info << "Number of lists in graph : "
            << graphe->size()
            << endl;
  map<int, RandomAccessSet<function_t> >::iterator it_graphe = graphe->begin();
  while(it_graphe != graphe->end())
  {
    ...
    if((*it_graphe).second.size() != 0)
    {
      ...
      */
      ...
      {
        RandomAccessSet<function_t>::iterator it_curr = (*it_graphe).second.begin();
