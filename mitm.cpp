...
extern RandomAccessVector<bool_op_t> b;
extern function_t start;
extern function_t arrival;
extern function_t unsorted_arrival;
extern map<int, RandomAccessSet<function_t> > f1_succ;
extern map<int, RandomAccessSet<function_t> > f2_succ;
...
void mitm(function_t f1, function_t f2)
{
  ...
  */
  ...
  f1.sort();
  f2.sort();
 RandomAccessSet<function_t> lf1;
  RandomAccessSet<function_t> lf2;
  lf1.insert(f1);
  lf2.insert(f2);
  f1_succ.insert(pair<int, RandomAccessSet<function_t>>(0, lf1));
  f2_succ.insert(pair<int, RandomAccessSet<function_t>>(0, lf2));
  start = f1;
  arrival = f2;
...
void expand(map<int, RandomAccessSet<function_t> > *current,
            map<int, RandomAccessSet<function_t> > *opposite,
            int lambda)
{          
  ...
  if( to_expand != current->end())
  {
    for(auto bool_op : ::b)
    {
      ...
      RandomAccessVector<function_t> successors;
      ...
      if(successors.size() != 0)
      {
        ...
        RandomAccessSet<function_t> new_list;
        if(s != current->end())
        { 
          ...
          for(auto item: new_list){
            s->second.insert(item);
          }

          std::cout << "s size : " << s->second.size() << " bool_op_const : " << bool_op_cost << std::endl;
        }
        else
        { 
           v_list_process(lambda, bool_op_cost, &successors, current, &new_list, opposite);
           current->insert(pair<int,RandomAccessSet<function_t>>(lambda+bool_op.op_cost, new_list));
           std::cout << "successors size : " << successors.size() << " bool_op_const : " << bool_op_cost << std::endl;
        }
      }
    }
  }
...
void v_list_process(int lambda, int op_cost, RandomAccessVector<function_t> *tmp,
                    map<int, RandomAccessSet<function_t> > *current,
                    RandomAccessSet<function_t> *to_insert, map<int,RandomAccessSet<function_t> > *opposite)
{
...
bool is_in_graphe(int lambda, int op_cost, function_t f, map<int, RandomAccessSet<function_t> > *g)
{
...
bool is_in_graphe_collision(int lambda, int op_cost, function_t f,
                            function_t *fun_found,
                            int *vect_found,
                            map<int, RandomAccessSet<function_t> > *g)
{
  for(auto it = g->begin(); it != g->end(); it++)
  {
